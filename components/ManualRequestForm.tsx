...
export const ManualRequestForm: React.FC<ManualRequestFormProps> = ({ onSubmit, onClose, initialData }) => {
    ...
    useEffect(() => {
        if (initialData) {
            // FIX: Correctly merge initialData into formData, respecting optional properties and defaults.
            setFormData(prev => ({
    ...
    // FIX: Update function signature to accept an optional event argument to handle calls from both form onSubmit and button onClick.
    ...
    return (
        // FIX: Wrap onClick handler in a lambda to prevent passing an event object.
        <div className="fixed inset-0 bg-black/60 z-[4000] flex justify-center items-center p-4" onClick={() => onClose()}>
            <div className="bg-gray-50 text-gray-900 rounded-lg shadow-xl w-full max-w-3xl animate-fade-in-up flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-4 border-t mt-auto flex-shrink-0 bg-gray-100 flex justify-end gap-2">
                     ...
                     {/* FIX: Changed button type to "button" and wrapped handleSubmit in a lambda to prevent unintended form submissions and type mismatches. */}
