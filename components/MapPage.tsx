...
const OcorrenciaDetailsPanel: React.FC<{
  ...
  const { ocorrencia, onClose, availableVolunteers, assignedVolunteers, allTeams, suggestedVolunteerIds, availableRecursos, chatMessages, user, onAssignVolunteers, onAssignTeam, onAssignRecursos, onSendMessage, onUpdatePriority, onUpdateStatus, onViewHistory, initialTab = 'details' } = props;
  ...
  const handleToggleRecurso = (id: string) => setSelectedRecursoIds(p => p.includes(id) ? p.filter(i => i !== id) : [...p, id]);
  
  const handleSubmitAssignment = () => {
    if (assignmentMode === 'individuals' && selectedVolunteerIds.length > 0) {
        onAssignVolunteers(selectedVolunteerIds);
    } else if (assignmentMode === 'teams' && selectedTeamId) {
        onAssignTeam(selectedTeamId);
    }
  };
  ...
  const filteredVolunteers = useMemo(() => {
    return availableVolunteers.filter(v =>
      !v.teamId && // Show only unassigned volunteers in individual mode
          ...
          <div className="p-4 space-y-4">
            ...
            {ocorrencia.status === 'Aberta' && (
              <div ref={setAssignmentSectionRef}><DetailSection title="Designar Voluntários" icon={<UserGroupIcon className="h-4 w-4"/>}>
                ...
                {assignmentMode === 'teams' && <ul className="space-y-1 max-h-48 overflow-y-auto">{availableTeams.map(t => (<li key={t.id}><label className={`flex items-center p-2 rounded-md cursor-pointer transition-colors ${selectedTeamId === t.id ? 'bg-cyan-900/50' : 'bg-gray-700/50'}`}><input type="radio" name="team-selection" checked={selectedTeamId === t.id} onChange={() => setSelectedTeamId(t.id)} className="hidden" /><div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center mr-2 flex-shrink-0 ${selectedTeamId === t.id ? 'border-cyan-400' : 'border-gray-500'}`}>{selectedTeamId === t.id && <div className="w-2 h-2 rounded-full bg-cyan-400"></div>}</div><div><p className="text-sm font-semibold">{t.name}</p><p className="text-xs text-gray-400">{t.memberIds.length} membros</p></div></label></li>))}</ul>}

                <button onClick={() => handleSubmitAssignment()} disabled={(assignmentMode === 'individuals' && selectedVolunteerIds.length === 0) || (assignmentMode === 'teams' && !selectedTeamId)} className="w-full mt-3 bg-cyan-600 hover:bg-cyan-700 text-white font-bold py-2 px-3 rounded-md text-sm disabled:bg-gray-700 disabled:cursor-not-allowed transition-colors">Designar</button>
              </DetailSection></div>
            )}
...
export const MapPage: React.FC<any> = (props) => {
  ...
  const [showVoluntarios, setShowVoluntarios] = useState(true);
  const [showOcorrencias, setShowOcorrencias] = useState(true);
  ...
  const [filterStatus, setFilterStatus] = useState<OcorrenciaStatus | 'all'>('Aberta');
  const [filterGravidade, setFilterGravidade] = useState<OcorrenciaGravidade[]>([]);
  const [isDashboardOpen, setDashboardOpen] = useState(true);
  const [isEventLogOpen, setEventLogOpen] = useState(true);
  ...
  const filteredOcorrencias = useMemo(() => {
    return ocorrencias.filter((o: Ocorrencia) => {
        if (!showOcorrencias) return false;
        const matchesTipo = filterTipo.length === 0 || filterTipo.includes(o.tipo);
        const matchesStatus = filterStatus === 'all' || o.status === filterStatus;
        const matchesGravidade = filterGravidade.length === 0 || filterGravidade.includes(o.gravidade || 'Moderado');
        return matchesTipo && matchesStatus && matchesGravidade;
    });
  }, [ocorrencias, showOcorrencias, filterTipo, filterStatus, filterGravidade]);
  ...
  const handleAssignTeam = (teamId: string) => {
    if (selectedOcorrencia) {
      assignTeamToOcorrencia(teamId, selectedOcorrencia.id, user.name);
      setSelectedOcorrencia(null);
    }
  };
      ...
      <div className={`fixed inset-y-0 left-0 w-80 z-[1100] bg-gray-800/90 backdrop-blur-md transition-transform duration-300 ease-in-out md:relative md:w-80 md:translate-x-0 flex-shrink-0 ${isLeftPanelVisible ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-full overflow-y-auto p-4 space-y-4">
          <MapControls
              ...
              showVoluntarios={showVoluntarios} setShowVoluntarios={setShowVoluntarios}
              showOcorrencias={showOcorrencias} setShowOcorrencias={setShowOcorrencias}
              ...
              heatmapRadius={heatmapRadius} setHeatmapRadius={setHeatmapRadius}
              ...
              onOpenSettingsPanel={() => setSettingsPanelOpen(true)} onToggleDashboard={() => setDashboardOpen(!isDashboardOpen)}
              isDashboardOpen={isDashboardOpen} onToggleEventLog={() => setEventLogOpen(!isEventLogOpen)}
              isEventLogOpen={isEventLogOpen} onOpenDataManagementPanel={() => setDataManagementPanelOpen(true)}
              ocorrenciaCount={filteredOcorrencias.length} riskZoneCount={riskZones.length}
              volunteerCount={voluntarios.length}
              filterTipo={filterTipo} onToggleTipoFilter={(tipo: string) => setFilterTipo(prev => prev.includes(tipo) ? prev.filter(t => t !== tipo) : [...prev, tipo])}
              filterStatus={filterStatus}
              onFilterStatusChange={(value) => setFilterStatus(value as OcorrenciaStatus | 'all')}
              filterGravidade={filterGravidade} onToggleGravidadeFilter={(g: OcorrenciaGravidade) => setFilterGravidade(prev => prev.includes(g) ? prev.filter(gr => gr !== g) : [...prev, g])}
          ...
          />
          {isDashboardOpen && <DashboardPanel voluntarios={voluntarios} ocorrencias={ocorrencias} criticalAlertsCount={criticalAlertsCount} />}
          <WeatherDisplay weatherData={weatherData} loading={weatherLoading} error={weatherError} />
          {isEventLogOpen && <EventLogPanel events={eventLog} onEventClick={(lat, lng) => mapControlRef.current?.panToLocation(lat, lng)} />}
        </div>
      </div>

      <div className="flex-grow h-full relative">
        <LeafletMap
          ref={mapControlRef} voluntarios={voluntarios.filter((v: Voluntario) => showVoluntarios)} ocorrencias={filteredOcorrencias}
          riskZones={showRiskZones ? riskZones : []} climateAlerts={climateAlerts} activation={activation}
          showHeatmap={showHeatmap} heatmapRadius={heatmapRadius} heatmapIntensity={heatmapIntensity}
          showRoutes={showRoutes} newestIncidentId={newestIncidentId}
