...
import { TableCellsIcon, XMarkIcon, PencilIcon, TrashIcon, UserGroupIcon, ExclamationTriangleIcon, PlusIcon, MagnifyingGlassIcon } from './icons';
...
const DataManagementPanel: React.FC<DataManagementPanelProps> = ({
  ...
  const handleSubmitVolunteer = (e: React.FormEvent<HTMLFormElement>) => {
      e.preventDefault();
      setFormError(null);
      ...
      if(editingVolunteer) {
          onUpdateVolunteer({...editingVolunteer, ...volunteerData}, operatorName);
      } else {
          onAddVolunteer(volunteerData, operatorName);
      }
      handleCloseForm();
  };

  const handleSubmitOcorrencia = (e: React.FormEvent<HTMLFormElement>) => {
      e.preventDefault();
      setFormError(null);
      const formData = new FormData(e.currentTarget);
      const data = Object.fromEntries(formData.entries());
      ...
      if (editingOcorrencia) {
          onUpdateOcorrencia({ ...editingOcorrencia, ...ocorrenciaData }, operatorName);
      } else {
          onAddOcorrencia(ocorrenciaData as Omit<Ocorrencia, 'id' | 'timestamp'>, operatorName);
      }
      handleCloseForm();
  };
