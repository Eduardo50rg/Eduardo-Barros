...
// FIX: Module '"file:///components/AdminPortal"' has no default export. Changed to named import.
...
const App: React.FC<AppProps> = ({ user, setAppUser, authLoading }) => {
    const realtimeData = useRealtimeMapData(user); // Pass user to hook for role-based data fetching
    ...
    const handleHelpRequest = async (requestData: HelpRequestData) => {
        ...
        // After submitting, we associate the new occurrence with the citizen user
        if (user && user.role === 'cidadao') {
             const citizenWithOcorrencia = { ...user, activeOcorrenciaId: newOcorrencia.id };
             setAppUser(citizenWithOcorrencia); // Update parent state
        }
    };
    
    // Sync offline requests when connection is back
    useEffect(() => {
        ...
        syncOfflineRequests(); // Also try on initial load
    ...
    // Find the occurrence associated with the current citizen user
    ...
    // Find the volunteers assigned to the citizen's occurrence
    ...
    const renderContent = () => {
        ...
        switch (user.role) {
            case 'cidadao':
                 // This is a special case where we might create a temporary user on the fly.
            ...
            case 'administrador':
                return <AdminPortal 
                        ...
                        />;
            default:
                 logOut(); // Log out unknown user roles
