...
const LeafletMap = forwardRef<MapControlHandles, LeafletMapProps>(({
  ...
  useEffect(() => {
    ...
    if (showHeatmap && heatmapLayerRef.current) {
      const points = (heatmapOcorrencias || ocorrencias).map(o => [o.latitude, o.longitude, 1]); // Intensity per point is 1
  ...
  }, [showHeatmap, ocorrencias, heatmapOcorrencias, heatmapRadius, heatmapIntensity]);
